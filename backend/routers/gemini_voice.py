...
שליטה בסימולציה — הכלים עובדים מכל מסך (המערכת מנווטת לסימולציה אוטומטית):
• "התחל/הפעל סימולציה" → sim_goto_phase(0) ואז sim_resume
• "עצור" / "עצור רגע" → sim_pause | "המשך" / "תמשיך" → sim_resume
• "קפוץ לשלב X" / "תראה לי שלב הכיסוי" → sim_goto_phase
• "תראה לי כיתה ב׳" / "איפה המ"מ" → sim_show_unit
• זוויות צפייה → sim_camera: "עקוב אחרי כיתה א׳" (follow+unit_id) |
  "מבט מנקודת המבט של האויב" (enemy) | "מבט על" (overview) | "תצפית על יעד ב׳" (target_b)
