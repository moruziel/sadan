...
SCREEN_GUIDE: dict = {
    ...
    "area": (
        ...
        "במצב גזרה (תצוגת גזרה מלאה) המשתמש רואה את שטחי גזרת הגולן 251: "
        "ש.א. 309ה (ציון התאמה 92 — מומלץ, פנוי), ש.א. 120 מרכז (85, תפוס עד 12.5), "
        "ש.א. אל פוראן (78, פנוי), ש.א. דלווה (72, פנוי), ש.א. משתא (68, פנוי). "
        "המלצה יזומה: הצע להראות את נקודות העניין בשטח (המנחת, המפגעים, היעדים). "
    ...
    "questionnaire": (
        ...
        "רמת כשירות (readiness: א/ב/ג/ד), תנאי ירי (firingCond: רטוב/יבש), תחמושת (ammo), "
        "תאריך (date), גודל כוח (forceSize), הרכב (composition), ושת\"פ יחידות. "
        "כלי: fill_field למילוי שדות בקול. "
        "בתרגיל רטוב — פינוי רכוב (4x4 + חובש) מתווסף אוטומטית לשת\"פ. "
        "הנימוק: באימון עם ירי חי חובה כוננות פינוי רפואי צמודה — זמן תגובה 3-7 דקות "
        "הוא ההבדל בין פציעה לאסון, ולכן המערכת לא מאפשרת תרגיל רטוב בלעדיו. "
        "המשך הזרימה: כפתור 'המשך' → מסך המתווים."
    ),
    "plans": (
        "מסך 3 מתווים מוצעים. המשתמש רואה 3 כרטיסים עם מפה מוקטנת, סיפור וציון התאמה. "
        "המתווים והנימוקים (פירוק הציון: התאמה למטרות 40%, בטיחות 30%, זמינות משאבים 20%, היסטוריה 10%): "
        "מתווה א' 'איגוף מדרום' — ציון 92, מומלץ. 3 ימים, סיכון בינוני, מבוסס תיק מדף מאומת. "
        "למה מומלץ: כיבוש רב-שלבי שמנצל את ארכיטקטורת השטח (רכס שולט מדרום), עונה על כל מטרות "
        "האימון (כיבוש בטונדות, תיאום חפ\"ק, ניהול אמל\"ח), והסיכון הדו\"צי מנוהל בהסטת אש. "
        "מתווה ב' 'חזיתי ברצף' — ציון 74. 2 ימים, סיכון נמוך, ללא חבלה. "
        "פשוט ובטוח, אבל ערך אימוני נמוך יותר — אין תרגול תמרון ואיגוף. "
        "מתווה ג' 'לילי חבלה' — ציון 81. 2 לילות, סיכון גבוה. ניווט 8 ק\"מ + הרס מבנה. "
        "ערך אימוני גבוה אך דורש אישור בטיחות נוסף בגלל חבלה בלילה. "
        "הדרכה: הסבר את השיקולים, ההחלטה של המפקד. בחירה → תיק תרגיל נבנה אוטומטית."
    ),
    "exercise": (
        "מסך תיק התרגיל — המסמך המלא שסדן בנתה. sidebar עם 8 טאבים, ותוכנם: "
        "כללי — הפקודה הכללית: 6 שלבי התרגיל מכינוס ותדריך (H-4) דרך התקפה (H+2) ועד תחקיר ונסיגה (H+6). "
        "בטיחות — הוראות נשק ומחסנית, קצין בטיחות סרן יואב שמיר, ו-2 דילמות בטיחות שסדן זיהתה: "
        "חירבה מתפוררת 12 מ' ממסלול ההסתערות (פתרון: הגבלת ציר + תחקיר הנדסה), "
        "ובור מים לא מגודר ליד יעד ב' (פתרון: סימון + רדיוס מגבלה 25 מ'). "
        "ירי ושטחים — אזימוטי ירי (גבולות 350°-035°), מיני-מפה, והמערכות המסונכרנות: "
        "קרקע בטוחה (הקצאה KB-2026-1847) ומאור (תחמושת). ממתין לאישור מתא\"ם H-48. "
        "נת\"בים — נקודות תורפה בטיחותיות בשטח. "
        "לוגיסטיקה — מים, מזון, הובלה, תחמושת. "
        "תדריך — תדריך המפקד. שת\"פ יחידות — פינוי רכוב + המסוק שנוסף בשאלון. "
        "נוהל קרב — תהליך העבודה של המ\"פ. "
        "כלי: sadan:open_tab לפתיחת טאב בקול. יש כפתורי ייצוא (מייל/יומן/PDF). "
        "חשוב: המעבר לבוחן נפתח רק אחרי צפייה בטאבים 'ירי ושטחים' ו'נת\"בים' — עיקרון בטיחות מכוון. "
        "אם המשתמש שואל למה הכפתור נעול — הסבר שחובה לעבור על נתוני הירי והנת\"בים לפני בוחן."
    ),
    "quiz": (
        "מסך בוחן הכנה — 8 שאלות אמריקאיות מתוך תיק התרגיל + שאלה פתוחה. "
        "זהו סף כניסה לשטח: מפקד שלא שולט בתיק לא נכנס לתרגיל. "
        "אפשר לענות בקול: 'שאלה 3 תשובה ב'. 'הגש בוחן' לסיום. "
            ...
            function_declarations=[
                types.FunctionDeclaration(
                    ...
                    description=(
                        ...
                        "  firingCond = תנאי ירי ('יבש'/'רטוב'/'שניהם') — כשהמשתמש אומר 'תרגיל רטוב'/'תנאי ירי רטוב'. "
                    ...
                    parameters=types.Schema(
                        ...
                        properties={
                            "field_id": types.Schema(
                                type=types.Type.STRING,
                                description=(
                                    "שאלון: readiness/firingCond/objective/topic/ammo/date/forceSize/composition. "
