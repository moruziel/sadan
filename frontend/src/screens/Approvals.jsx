...
export default function Approvals() {
  ...
  // ── שיחה טלפונית ────────────────────────────────────────
  const [callStatus, setCallStatus] = useState({}) // partyId → 'calling' | 'done' | null
  ...
  async function makePhoneCall(partyId) {
    ...
    try {
      ...
      if (data.status === 'calling') {
        ...
        setResponses(prev => ({ ...prev, [partyId]: `📞 שיחה יוצאת — מזהה: ${data.call_id?.slice(0,8)}` }))
      } else {
                ...
                <div className="space-y-4">
                  ...
                  {selectedParty.status === 'pending' && (
                    <div className="flex gap-2 flex-wrap">
                      ...
                      {hasScript && (
                        <button
                          onClick={() => sendVoiceRequest(selectedParty.id)}
                          disabled={isVoiceSending}
                          className="flex items-center gap-2 px-4 py-2.5 bg-blue-900/30 border border-blue-500/30 text-blue-300 hover:bg-blue-900/50 font-semibold rounded-xl text-sm transition-all disabled:opacity-50"
                        >
                          {isVoiceSending
                      ...
                      {hasScript && (
                        <button
                          ...
                          disabled={callStatus[selectedParty.id] === 'calling'}
                          className="flex items-center gap-2 px-4 py-2.5 bg-green-900/30 border border-green-500/30 text-green-300 hover:bg-green-900/50 font-semibold rounded-xl text-sm transition-all disabled:opacity-50"
                        >
                  ...
                  {/* תגובה טקסט */}
                  {resp && (
                    <div className="bg-demo-success/10 border border-demo-success/30 rounded-xl px-4 py-3 text-demo-success text-sm font-medium">
                      {resp}
                    </div>
                  )}

                  {/* Transcript הודעה קולית */}
                  {voiceTx && (
                    <div className="bg-[#1e2d45] border border-blue-500/20 rounded-xl p-4 space-y-2">
                      <div className="flex items-center gap-2 text-blue-300 text-xs font-bold">
                        <Volume2 size={13} />
                        <span>סדן (הודעה קולית) — {voiceTx.time}</span>
                      </div>
                      <p className="text-blue-200 text-xs leading-relaxed italic">"{voiceTx.text}"</p>
